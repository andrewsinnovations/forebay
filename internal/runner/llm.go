...
func executeLLMTask(ctx context.Context, db *store.DB, task *store.Task, tee bool) {
	...
	go func() {
		...
		resultCh <- callResult{content, ex, err}
	}()
	...
	for {
		...
		case res := <-resultCh:
			fmt.Fprintf(logFile, "request:\n%s\n\nraw response:\n%s\n",
				res.exchange.Request, res.exchange.Response)
			if res.err != nil {
			...
			// Structured output was requested but the endpoint replied
			// with prose; the reply is still saved as the result.
			if len(spec.Schema) > 0 && !json.Valid([]byte(strings.TrimSpace(res.content))) {
				...
				db.MarkFinished(task.ID, store.StatusFailed, -1, msg, res.content)
				fmt.Printf("[%s/%s] failed (%s): %s\n",
					batch.Name, task.ID, time.Since(start).Round(time.Second), msg)
				return
			}
